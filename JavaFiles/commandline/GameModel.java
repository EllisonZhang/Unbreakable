...
// Model containing the data and the method necessary to run a game
	...
	// Name of the winner of the game
	String gameWinner;
	// Boolean enabling use to close a game
	boolean gameOver = false;
	// number of rounds
	int round;
	// Attribute chosen at each round
	...
	public boolean round() {
		...
		for (Player player : players) {
			...
			if (!player.getDeck().isEmpty()) {
				if (player.human) {
					...
					System.out.println("There are " + (player.getDeck().size()) + " cards in your deck");
		...
		if (board.values().size() == 1) {
			...
			gameWinner = choosingPlayer.name;
			gameOver = true;
			return false;
		}
		...
		draw = comparison(board);
		return draw;

	}

	public boolean round(String attributeValue) {
		/*
		 * At the beginning of each round we create a board containing the players and
		 * their top cards. (Only for the players that have remaining cards in their
		 * deck).
		...
		choosingValue(attributeValue, board);
		draw = comparison(board);
		return draw;
	...
	public void choosingValue(String option, HashMap<Player, Card> board) {
		 ...
		 */
		System.out.println("Chosen category: " + option);
